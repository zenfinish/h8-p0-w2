...
function getReport(studentGrades) {
  ...
  for(var i = 0; i < studentGrades.length; i++) {
    group[studentGrades[i].classCode] = [];
  }

  var objGroup = Object.keys(group);
  for(var i = 0; i < objGroup.length; i++) {
    
    var pass = [];
    var fail = [];
    for(var j = 0; j < studentGrades.length; j++) {
      if(objGroup[i] === studentGrades[j].classCode) {
        if(studentGrades[j].score >= 70) {
          pass.push(studentGrades[j].name);
        }else{
          fail.push(studentGrades[j].name);
        }
      }
    }

    output.push({
      classCode: objGroup[i],
      passed: pass,
      failed: fail
    });
